...
/**
 * Pure-core tests for the per-scope seeds (repo / capability / org). Each builds
...
import type { RepoKnowledge, CapabilityKnowledge, OrgKnowledge } from "@/lib/api/client";
...
describe("scope id helpers", () => {
  it("round-trips synthetic ids", () => {
    ...
    expect(parseScopeId("scope:capability:xyz")).toEqual({ kind: "capability", id: "xyz" });
...
describe("seedCapability", () => {
  it("seeds repos + top_entities + entity edges", () => {
    const cap = {
      capability_id: "c1", nodes_total: 0, nodes_by_kind: {}, edges_total: 0, repos_indexed: 1,
    ...
    } as unknown as CapabilityKnowledge;
    const s = seedCapability(cap, { name: "Billing", repos: [{ id: "r1", name: "acme/billing" }] });
    expect(s.rootId).toBe("scope:capability:c1");
...
describe("seedOrg", () => {
  it("seeds capability refs + cross-cap edges", () => {
    const org = {
      org_id: "o1",
      capabilities: [
      ...
      cross_cap_dependencies: [{ from_capability_id: "c1", to_capability_id: "c2", kind: "data", label: "events", evidence: [] }],
    } as unknown as OrgKnowledge;
    const s = seedOrg(org, { name: "Acme" });
    expect(s.rootId).toBe("scope:org:o1");
    expect(s.nodes.map((nd) => nd.id)).toContain("scope:capability:c1");
    expect(s.edges.some((e) => e.source_id === "scope:capability:c1" && e.target_id === "scope:capability:c2")).toBe(true);
  });
});
