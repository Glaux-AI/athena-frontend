 ...
 * ``(dom_inbox, repo_n1)`` pair so the mock handler resolves the repo
 * via its in-memory ``domainRepos`` registry.
 */
import { describe, expect, it } from "vitest";

import { api } from "@/lib/api/client";
import { domainRepos } from "@/lib/api/mock/db";

describe("api.domains.retryRepoEnrichments", () => {
  it("returns the parsed response shape with per-kind breakdown", async () => {
    ...
    const repo = domainRepos.dom_inbox?.[0];
    if (!repo) throw new Error("seed missing");
    repo.current_sync_stage = "degraded";

    const out = await api.domains.retryRepoEnrichments(
      "dom_inbox", repo.id,
    );
  ...
  it("accepts an explicit kinds[] subset", async () => {
    const repo = domainRepos.dom_inbox?.[1];
    if (!repo) throw new Error("seed missing");
    repo.current_sync_stage = "degraded";

    const out = await api.domains.retryRepoEnrichments(
      "dom_inbox", repo.id, { kinds: ["embedding", "tag"] },
    );
