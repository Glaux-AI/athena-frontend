 ...
 * (`dom_billing`, slug `billing`) because the mock seeds it with
...
test.describe("§5.31.9 r3 Soft-delete lifecycle", () => {
  test("soft-delete -> trash -> restore -> permanent-delete", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /soft delete this domain/i }),
    ...
    const trashCapText = page.getByText(`dom:${CAP_SLUG}`);
     ...
     * `dom:{slug}`. The mock restore endpoint clears `deleted_at`.
     * ------------------------------------------------------------------ */
    const capCard = page
      .locator("div", { has: page.getByText(`dom:${CAP_SLUG}`) })
      .first();
