...
export function CapabilityDangerZoneTab({ cap, canManage, onChanged }: Props) {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Destructive actions on this capability are restricted to
  ...
  return (
    <Stack gap="4">
      <SoftDeleteCard
        cap={cap}
        onSuccess={() => {
          router.push("/capabilities?status=deleted");
        }}
      />
    </Stack>
  );
...
function SoftDeleteCard({
  ...
  const onSubmit = async () => {
    ...
    setError(null);
    try {
      await api.capabilities.softDelete(cap.id);
      toast.success(`Capability "${cap.name}" moved to trash.`);
      onSuccess();
    } catch (e) {
    ...
    <Card className="border-[var(--danger)] shadow-[var(--shadow-2)]">
      <CardHeader>
        ...
        <CardDescription>
          ...
          knowledge graph all exclude soft-deleted capabilities. You can
...
function DeletedBanner({ cap }: { cap: Capability }) {
...
function RestoreCard({
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      toast.success(
        `Capability "${cap.name}" restored. Re-ingest enqueued for every attached repo.`,
      );
    ...
    <Card className="border-[var(--danger)] shadow-[var(--shadow-2)]">
      <CardHeader>
        ...
        <CardDescription>
          ...
          it: attached repos&apos; join rows, capability memberships,
          capability skills, the cap&apos;s Blueprint sections, and every
          knowledge node / edge scoped to this cap.{" "}
