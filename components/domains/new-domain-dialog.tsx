 ...
 * description → `api.domains.create` → toast + `onCreated`. Slug
...
interface Props {
  ...
  /** Called once after a successful create with the new domain so the
   *  parent can refresh its list and/or navigate to the new cap. */
  onCreated: (cap: Domain) => void;
}
...
export function NewDomainDialog({ open, onOpenChange, onCreated }: Props) {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.field === "slug") {
        ...
        setSlugError(err instanceof ApiError ? err.message : "Couldn't create domain.");
      }
    } finally {
      setBusy(false);
    }
