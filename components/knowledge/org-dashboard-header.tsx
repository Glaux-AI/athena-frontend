 ...
 *
 * Domain links come from the org `portfolio` Blueprint section's
...
export function OrgDashboardHeader({ orgId, orgKnowledge }: OrgDashboardHeaderProps) {
  ...
  // Prefer the section's `domains` link list; fall back to the registry.
  const caps = portfolio?.domains ?? orgKnowledge?.domains.map((c) => ({ domain_id: c.id, name: c.name })) ?? [];
      ...
      <Stack gap="3">
        ...
        <ul className="grid grid-cols-1 gap-2 md:grid-cols-2">
          {caps.map((c) => (
            <li key={c.domain_id}>
              <Link
                href={`/domains/${encodeURIComponent(c.domain_id)}`}
